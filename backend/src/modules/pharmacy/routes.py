...
async def list_pharmacies(
    ...
    try:
        return await service.list_providers(
            db=db, city=city, delivery=delivery, skip=compute_offset(page, items_per_page), limit=items_per_page
        )
...
async def create_order(
    data: PharmacyOrderCreate, current_user: CurrentUserDep, db: AsyncSessionDep, service: PharmacyServiceDep
) -> dict[str, Any]:
    try:
...
async def list_my_orders(current_user: CurrentUserDep, db: AsyncSessionDep, service: PharmacyServiceDep) -> dict[str, Any]:
    try:
...
async def get_order(
    order_id: int, current_user: CurrentUserDep, db: AsyncSessionDep, service: PharmacyServiceDep
) -> dict[str, Any]:
    try:
...
async def create_provider(
    _: CurrentSuperUserDep, data: PharmacyProviderCreate, db: AsyncSessionDep, service: PharmacyServiceDep
) -> dict[str, Any]:
    try:
...
async def create_medicine(
    _: CurrentSuperUserDep, data: MedicineCreate, db: AsyncSessionDep, service: PharmacyServiceDep
) -> dict[str, Any]:
    try:
