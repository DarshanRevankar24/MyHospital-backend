...
from .schemas import MedicineCreate, MedicineRead, PharmacyOrderCreate, PharmacyOrderRead, PharmacyProviderCreate, PharmacyProviderRead
...
class PharmacyService:
    async def list_providers(self, db: AsyncSession, city: str | None = None, delivery: bool | None = None,
                             skip: int = 0, limit: int = 20) -> dict[str, Any]:
        filters: dict[str, Any] = {"is_active": True, "is_verified": True}
        if city: filters["city"] = city
        if delivery is not None: filters["is_delivery_available"] = delivery
        return await crud_pharmacy_providers.get_multi(db=db, offset=skip, limit=limit,
                                                       schema_to_select=PharmacyProviderRead, **filters)

    async def get_provider(self, provider_id: int, db: AsyncSession) -> dict[str, Any]:
        p = await crud_pharmacy_providers.get(db=db, id=provider_id, is_active=True,
                                              schema_to_select=PharmacyProviderRead)
        if not p: raise ResourceNotFoundError("Pharmacy not found")
        return p
    ...
    async def create_provider(self, data: PharmacyProviderCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        if not created: raise ValidationError("Failed to create pharmacy")
        return created

    async def list_medicines(self, provider_id: int, db: AsyncSession, category: str | None = None,
                             name: str | None = None) -> dict[str, Any]:
        filters: dict[str, Any] = {"provider_id": provider_id, "is_available": True}
        if category: filters["category"] = category
        return await crud_medicines.get_multi(db=db, offset=0, limit=200, schema_to_select=MedicineRead, **filters)

    async def search_medicines(self, db: AsyncSession, name: str) -> dict[str, Any]:
        return await crud_medicines.get_multi(db=db, offset=0, limit=50, schema_to_select=MedicineRead, is_available=True)
    ...
    async def create_medicine(self, data: MedicineCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        if not created: raise ValidationError("Failed to add medicine")
        return created
    ...
    async def create_order(self, user_id: int, data: PharmacyOrderCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        if not provider: raise ResourceNotFoundError("Pharmacy not found")
        ...
        # Deduct stock (simple approach for now)
        for item in data.items:
            med = await crud_medicines.get(db=db, id=item.medicine_id)
            if med:
                await crud_medicines.update(db=db, object={"stock_quantity": med["stock_quantity"] - item.quantity}, id=med["id"])

        from fastcrud import FastCRUD
        from .models import PharmacyOrder
        created = await FastCRUD(PharmacyOrder).create(db=db, object=order_data, schema_to_select=PharmacyOrderRead)
        if not created: raise ValidationError("Failed to place order")
        return created

    async def list_user_orders(self, user_id: int, db: AsyncSession) -> dict[str, Any]:
        return await crud_pharmacy_orders.get_multi(db=db, offset=0, limit=50, schema_to_select=PharmacyOrderRead, user_id=user_id)
    ...
    async def get_order(self, order_id: int, user_id: int, db: AsyncSession) -> dict[str, Any]:
        ...
        if not o: raise ResourceNotFoundError("Order not found")
        return o
    ...
    async def cancel_order(self, order_id: int, user_id: int, db: AsyncSession) -> None:
        ...
        if not o: raise ResourceNotFoundError("Order not found")
        if o["status"] not in ("pending",): raise ValidationError("Only pending orders can be cancelled")
