...
from .schemas import FamilyConnectionRead, FamilyRequestCreate
...
class FamilyService:
    ...
    async def send_family_request(
        ...
        logger.info(
            f"Verification link generated for {identifier}: https://app.myhospital.com/verify-family?conn={created_conn['id']}"
        )
    ...
    async def get_family_members(self, user_id: int, db: AsyncSession) -> list[dict[str, Any]]:
        ...
        all_connections: list[dict[str, Any]] = req_data + rec_data
