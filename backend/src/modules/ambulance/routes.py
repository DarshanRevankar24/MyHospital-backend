...
async def list_providers(db: AsyncSessionDep, service: AmbulanceServiceDep,
...
async def find_available(db: AsyncSessionDep, service: AmbulanceServiceDep,
                         city: str | None = Query(default=None)) -> dict[str, Any]:
    try:
...
async def create_request(data: AmbulanceRequestCreate, current_user: CurrentUserDep,
                         db: AsyncSessionDep, service: AmbulanceServiceDep) -> dict[str, Any]:
    try:
...
async def list_my_requests(current_user: CurrentUserDep, db: AsyncSessionDep,
                           service: AmbulanceServiceDep) -> dict[str, Any]:
    try:
...
async def cancel_request(request_id: int, current_user: CurrentUserDep, db: AsyncSessionDep,
                         service: AmbulanceServiceDep) -> dict[str, str]:
    try:
...
async def create_provider(_: CurrentSuperUserDep, data: AmbulanceProviderCreate,
                          db: AsyncSessionDep, service: AmbulanceServiceDep) -> dict[str, Any]:
    try:
...
async def create_vehicle(_: CurrentSuperUserDep, data: AmbulanceVehicleCreate,
                         db: AsyncSessionDep, service: AmbulanceServiceDep) -> dict[str, Any]:
    try:
