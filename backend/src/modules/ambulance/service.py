...
from .schemas import (AmbulanceProviderCreate, AmbulanceProviderRead, AmbulanceRequestCreate,
                      AmbulanceRequestRead, AmbulanceVehicleCreate, AmbulanceVehicleRead)
...
class AmbulanceService:
    async def list_providers(self, db: AsyncSession, city: str | None = None,
        ...
        return await crud_ambulance_providers.get_multi(db=db, offset=skip, limit=limit,
                                                        schema_to_select=AmbulanceProviderRead, **filters)

    async def get_provider(self, provider_id: int, db: AsyncSession) -> dict[str, Any]:
        p = await crud_ambulance_providers.get(db=db, id=provider_id, is_active=True,
                                               schema_to_select=AmbulanceProviderRead)
        if not p: raise ResourceNotFoundError("Ambulance provider not found")
        return p
    ...
    async def create_vehicle(self, data: AmbulanceVehicleCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        if not created: raise ValidationError("Failed to add vehicle")
        return created

    async def list_available_vehicles(self, db: AsyncSession, city: str | None = None) -> dict[str, Any]:
        # For a real implementation, this would involve geospatial queries.
        # This is simplified for the boilerplate.
        # It should join provider table to check city, but using fastcrud direct for now
        return await crud_ambulance_vehicles.get_multi(db=db, offset=0, limit=50,
                                                       schema_to_select=AmbulanceVehicleRead, is_available=True)
    ...
    async def create_request(self, user_id: int, data: AmbulanceRequestCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        created = await FastCRUD(AmbulanceRequest).create(db=db, object=req_data, schema_to_select=AmbulanceRequestRead)
        if not created: raise ValidationError("Failed to create request")
        
        # In a real system, here we would trigger a background task to find nearest available ambulance
        # and assign it.
        
        return created

    async def list_user_requests(self, user_id: int, db: AsyncSession) -> dict[str, Any]:
        return await crud_ambulance_requests.get_multi(db=db, offset=0, limit=50,
                                                       schema_to_select=AmbulanceRequestRead, user_id=user_id)
