...
from .schemas import LabBookingCreate, LabBookingRead, LabTestCreate, LabTestRead, LaboratoryProviderCreate, LaboratoryProviderRead
...
class LaboratoryService:
    ...
    async def list_providers(self, db: AsyncSession, city: str | None = None,
                             home_collection: bool | None = None, skip: int = 0, limit: int = 20) -> dict[str, Any]:
        ...
        return await crud_lab_providers.get_multi(db=db, offset=skip, limit=limit,
                                                   schema_to_select=LaboratoryProviderRead, **filters)
    ...
    async def list_tests(self, provider_id: int, db: AsyncSession, category: str | None = None,
                         name: str | None = None) -> dict[str, Any]:
        filters: dict[str, Any] = {"provider_id": provider_id, "is_available": True}
        if category:
            filters["category"] = category
        return await crud_lab_tests.get_multi(db=db, offset=0, limit=200, schema_to_select=LabTestRead, **filters)

    async def search_tests(self, db: AsyncSession, name: str) -> dict[str, Any]:
        return await crud_lab_tests.get_multi(db=db, offset=0, limit=50,
                                              schema_to_select=LabTestRead, is_available=True)
    ...
    async def create_booking(self, user_id: int, data: LabBookingCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        created = await FastCRUD(LabBooking).create(db=db, object=booking_data, schema_to_select=LabBookingRead)
        if not created:
            raise ValidationError("Failed to create booking")
        return created

    async def list_user_bookings(self, user_id: int, db: AsyncSession) -> dict[str, Any]:
        return await crud_lab_bookings.get_multi(db=db, offset=0, limit=50,
                                                  schema_to_select=LabBookingRead, user_id=user_id)
