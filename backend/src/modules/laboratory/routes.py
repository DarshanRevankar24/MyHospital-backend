...
from .schemas import LabBookingCreate, LaboratoryProviderCreate, LabTestCreate
...
async def list_labs(
    db: AsyncSessionDep,
    lab_service: LaboratoryServiceDep,
    ...
    try:
        return await lab_service.list_providers(
            db=db, city=city, home_collection=home_collection, skip=compute_offset(page, items_per_page), limit=items_per_page
        )
...
async def list_my_bookings(
    current_user: CurrentUserDep, db: AsyncSessionDep, lab_service: LaboratoryServiceDep
) -> dict[str, Any]:
    try:
...
async def get_booking(
    booking_id: int, current_user: CurrentUserDep, db: AsyncSessionDep, lab_service: LaboratoryServiceDep
) -> dict[str, Any]:
    try:
...
async def create_provider(
    _: CurrentSuperUserDep, data: LaboratoryProviderCreate, db: AsyncSessionDep, lab_service: LaboratoryServiceDep
) -> dict[str, Any]:
    try:
...
async def create_test(
    _: CurrentSuperUserDep, data: LabTestCreate, db: AsyncSessionDep, lab_service: LaboratoryServiceDep
) -> dict[str, Any]:
    try:
