...
class HospitalService:
    ...
    async def list_providers(
        self, db: AsyncSession, city: str | None = None, speciality: str | None = None,
        skip: int = 0, limit: int = 20,
    ) -> dict[str, Any]:
        filters: dict[str, Any] = {"is_active": True, "is_verified": True}
        if city:
            filters["city"] = city
        return await crud_hospital_providers.get_multi(
    ...
    async def list_doctors(
        self, provider_id: int, db: AsyncSession, speciality: str | None = None
    ) -> dict[str, Any]:
        ...
        return await crud_hospital_doctors.get_multi(
            db=db, offset=0, limit=100, schema_to_select=HospitalDoctorRead, **filters
        )
    ...
    async def list_slots(
        self, provider_id: int, doctor_id: int, slot_date: str, db: AsyncSession
    ) -> dict[str, Any]:
        ...
        return await crud_hospital_slots.get_multi(
            db=db, offset=0, limit=100, schema_to_select=HospitalSlotRead, **filters
        )
    ...
    async def create_booking(
        self, user_id: int, data: HospitalBookingCreate, db: AsyncSession
    ) -> dict[str, Any]:
        # Validate slot
        ...
        crud = FastCRUD(HospitalBooking)
        created = await crud.create(db=db, object=booking_data, schema_to_select=HospitalBookingRead)
        if not created:
            raise ValidationError("Failed to create booking")
        return created

    async def list_user_bookings(self, user_id: int, db: AsyncSession) -> dict[str, Any]:
        return await crud_hospital_bookings.get_multi(
    ...
    async def get_booking(self, booking_id: int, user_id: int, db: AsyncSession) -> dict[str, Any]:
        booking = await crud_hospital_bookings.get(
            db=db, id=booking_id, user_id=user_id, schema_to_select=HospitalBookingRead
        )
        if not booking:
    ...
    async def cancel_booking(self, booking_id: int, user_id: int, db: AsyncSession) -> None:
        ...
        await crud_hospital_bookings.update(
            db=db, object={"status": "cancelled"}, id=booking_id
        )
        # Release slot
        slot = await crud_hospital_slots.get(db=db, id=booking["slot_id"])
        if slot and slot["booked_count"] > 0:
            await crud_hospital_slots.update(
                db=db, object={"booked_count": slot["booked_count"] - 1}, id=booking["slot_id"]
            )
