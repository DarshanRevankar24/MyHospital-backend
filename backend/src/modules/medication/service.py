...
class MedicationService:
    ...
    async def get_user_medications(
        self, user_id: int, db: AsyncSession, limit: int = 100, offset: int = 0
    ) -> list[dict[str, Any]]:
        """Get all medications for a user."""
        res = await crud_medications.get_multi(
            db=db, user_id=user_id, is_deleted=False, schema_to_select=MedicationRead, limit=limit, offset=offset
        )
