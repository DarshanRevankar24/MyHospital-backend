...
async def list_blood_banks(db: AsyncSessionDep, service: BloodBankServiceDep,
...
async def search_stock(blood_group: str = Query(min_length=1), db: AsyncSessionDep = None,
                        service: BloodBankServiceDep = None) -> dict[str, Any]:
    try:
...
async def create_request(data: BloodRequestCreate, current_user: CurrentUserDep,
                          db: AsyncSessionDep, service: BloodBankServiceDep) -> dict[str, Any]:
    try:
...
async def list_my_requests(current_user: CurrentUserDep, db: AsyncSessionDep,
                            service: BloodBankServiceDep) -> dict[str, Any]:
    try:
...
async def get_request(request_id: int, current_user: CurrentUserDep, db: AsyncSessionDep,
                       service: BloodBankServiceDep) -> dict[str, Any]:
    try:
...
async def cancel_request(request_id: int, current_user: CurrentUserDep, db: AsyncSessionDep,
                          service: BloodBankServiceDep) -> dict[str, str]:
    try:
...
async def create_provider(_: CurrentSuperUserDep, data: BloodBankProviderCreate,
                           db: AsyncSessionDep, service: BloodBankServiceDep) -> dict[str, Any]:
    try:
...
async def upsert_stock(_: CurrentSuperUserDep, data: BloodStockCreate,
                        db: AsyncSessionDep, service: BloodBankServiceDep) -> dict[str, Any]:
    try:
