...
class BloodBankService:
    async def list_providers(self, db: AsyncSession, city: str | None = None, skip: int = 0, limit: int = 20) -> dict[str, Any]:
        ...
        if city:
            filters["city"] = city
        res = await crud_blood_bank_providers.get_multi(
            db=db, offset=skip, limit=limit, schema_to_select=BloodBankProviderRead, **filters
        )
        return dict(res)

    async def get_provider(self, provider_id: int, db: AsyncSession) -> dict[str, Any]:
        p = await crud_blood_bank_providers.get(db=db, id=provider_id, is_active=True, schema_to_select=BloodBankProviderRead)
        if not p:
            raise ResourceNotFoundError("Blood bank not found")
        return dict(p)
    ...
    async def get_stock(self, provider_id: int, db: AsyncSession) -> dict[str, Any]:
        ...
        res = await crud_blood_stocks.get_multi(
            db=db, offset=0, limit=20, schema_to_select=BloodStockRead, provider_id=provider_id
        )
        return dict(res)

    async def search_blood_group(self, blood_group: str, db: AsyncSession) -> dict[str, Any]:
        res = await crud_blood_stocks.get_multi(
            db=db, offset=0, limit=50, schema_to_select=BloodStockRead, blood_group=blood_group
        )
        return dict(res)
    ...
    async def list_user_requests(self, user_id: int, db: AsyncSession) -> dict[str, Any]:
        res = await crud_blood_requests.get_multi(db=db, offset=0, limit=50, schema_to_select=BloodRequestRead, user_id=user_id)
        return dict(res)
